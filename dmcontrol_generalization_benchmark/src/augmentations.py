...
class RandomOverlay(object):
	def __init__(self, dataset='places365_standard', batch_size=256, image_size=64):
		global places_iter
		self.alpha = 0.5
		if dataset == 'places365_standard':
			if places_dataloader is None:
				_load_places(batch_size=batch_size, image_size=image_size)
			self.imgs = _get_places_batch(batch_size=batch_size).repeat(1, 1, 1, 1)
		else:
			raise NotImplementedError(f'overlay has not been implemented for dataset "{dataset}"')

	def __call__(self, x):
		return ((1 - self.alpha) * (x / 255.) + (self.alpha) * self.imgs) * 255.
