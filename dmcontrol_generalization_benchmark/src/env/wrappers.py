...
class ColorWrapper(gym.Wrapper):
	...
	def _load_colors(self):
		...
		self._colors = torch.load(f'dmcontrol_generalization_benchmark/src/env/data/{self._mode}.pt')
	...
	def reload_physics(self, setting_kwargs=None, state=None):
		from dmcontrol_generalization_benchmark.src.env.dm_control.dm_control.suite import common
...
class DMCObsWrapper(gym.Wrapper):
	"""Make the env attributes look like maniskill to simplify the code"""
	def __init__(self, env):
		super().__init__(env)
		self.observation_space = self.init_observation_space()

	def init_observation_space(self):
		rgb_space = self.env.observation_space
		state_space = spaces.Box(0, 0, shape=(0,))
		return spaces.Dict({"rgbd": rgb_space, "state": state_space})

	def convert_observation(self, obs):
		obs = np.array(obs)
		return dict(rgbd=obs, state=np.array([0]))
