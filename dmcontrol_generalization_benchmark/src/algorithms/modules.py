...
from dmcontrol_generalization_benchmark.src.algorithms.vit import VisionTransformer
...
class SharedTransformer(nn.Module):
    def __init__(self, obs_shape, num_latents, patch_size=8, embed_dim=128, depth=4, num_heads=8, mlp_ratio=1., qvk_bias=False):
        super().__init__()
        assert len(obs_shape) == 3
        ...
        self.linear_head = nn.Linear(in_features=embed_dim, out_features=num_latents).cuda()
        self.out_shape = _get_out_shape_cuda(obs_shape, nn.Sequential(self.transformer, self.linear_head))
