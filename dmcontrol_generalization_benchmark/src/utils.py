...
from dmcontrol_generalization_benchmark.src import augmentations
...
def load_config(key=None):
    path = os.path.join('dmcontrol_generalization_benchmark/setup', 'config.cfg')
    with open(path) as f:
        data = json.load(f)
    if key is not None:
        return data[key]
...
class ReplayBuffer(object):
    ...
    def load(self, path):
        with np.load(path, allow_pickle=True) as buffer:
            for key in self.__dict__.keys():
                setattr(self, key, buffer[key])
        # Set last done to true
        if self.idx > 0 or self.full:
            self.not_dones[self.idx - 1] = 0

    def _get_samples(self, batch_inds):
        obs = self._obses[batch_inds]
        return obs

    # def iterate(self, batch_size):
    #     all_start_inds = np.arange(0, len(self) - 1, 1)
    #     if self.full:
    #         all_start_inds = (all_start_inds + self.idx) % len(self)
    ...
    #         batch_inds = np.stack([np.arange(s, s + 1) for s in start_inds], 0)
    #         batch_inds = batch_inds.transpose().reshape(-1)
    #         if self.full:
    #             # Prevent sampling across end of buffer
    #             batch_inds = (batch_inds + self.idx) % len(self)
    #         batch = self._get_samples(batch_inds)
    #         batch = [
    #             data.reshape(batch_size, *data.shape[1:]) for data in batch
    #         ]
    #         yield batch

    def iterate(self, batch_size):
        rand_inds = torch.randperm(len(self))
        for i in range(0, len(rand_inds) - batch_size, batch_size):
            batch_inds = rand_inds[i : i + batch_size]
            if self.full:
                # Prevent sampling across end of buffer
                batch_inds = (batch_inds + self.idx) % len(self)
            batch = self._get_samples(batch_inds.numpy())
